...
/** Implementation of [[https://tools.ietf.org/html/rfc6901 RFC6901 Json Pointers]]. This wraps
  * [[io.vertx.core.json.pointer.JsonPointer]] and makes it behave more Scala-like.
  * @param internal
...
final case class JsonPointer(private val internal: JJsonPointer):
  ...
  /** @return
    *   `true` if this pointer is a parent of the given `child`. For instance, `"/properties"` pointer is parent pointer
    *   of `"/properties/parent"`
    */
  ...
  /** Query the given `jsonObject`, expecting the result to be of type [[T]].
    *
    * <p>Note: If this pointer is a root pointer ("") and [[T]] is [[JsonObject]], this function returns the provided
    * JSON.</p>
    *
    * <p>Note: JSON numbers will always result in the [[Double]] Scala type. The following JSON to Scala mappings apply:
    * <ul> <li>`true/false` - `Boolean`</li> <li>`number` - `Double`</li> <li>`string` - `String`</li> <li>`array` -
    * [[JsonArray]]</li> <li>`object` - [[JsonObject]]</li> <li>`null` - will always result in `None`</li> </ul> </p>
    *
    * @param jsonObject
    *   the JSON to query
    * @tparam T
  ...
  /** Write `newElement` to the given `json` using this pointer. The path token "-" is handled as append to the end of
    * an array.
    *
    ...
    */
  def write[J <: JsonObject | JsonArray: Typeable](
      json: J,
      newElement: Any,
...
object JsonPointer:
  ...
  /** Builds a [[JsonPointer]] from a String, returning an [[Option]]. This comes in handy in situations when it is
    * unknown if the String contains a valid JSON pointer, e.g. if the String is a variable.
    *
    * @param pointer
    *   the String representing a pointer
    * @return
    *   [[Some Some(JsonPointer)]] if the given String was valid, else [[None]]
    */
  def fromOption(pointer: String): Option[JsonPointer] =
    try Some(JsonPointer(io.vertx.core.json.pointer.JsonPointer.from(pointer)))
    catch case _: IllegalArgumentException => None

  /** Builds a [[JsonPointer]] from a URI, returning an [[Option]]. This comes in handy in situations when it is unknown
    * if the URI contains a valid JSON pointer, e.g. if the URI is a variable.
    *
    * @param uri
    *   the uri representing the JSON pointer
    * @return
    *   [[Some Some(JsonPointer)]] if the given URI was valid, else [[None]]
    */
  def fromURIOption(uri: URI): Option[JsonPointer] =
    try Some(JsonPointer(io.vertx.core.json.pointer.JsonPointer.fromURI(uri)))
    catch case _: IllegalArgumentException => None
