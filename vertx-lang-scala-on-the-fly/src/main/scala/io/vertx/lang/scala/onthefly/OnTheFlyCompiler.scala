...
/** This compiler is used to compile both scriplets and scala-files contained in the classpath. The original class has
  * been taken from https://github.com/twitter/util/blob/master/util-eval/src/main/scala/com/twitter/util/Eval.scala. It
  * was modified in this blog post https://eknet.org/main/dev/runtimecompilescala.html
  *
...
class OnTheFlyCompiler(targetDir: Option[File]) {
  ...
  val classLoader = new AbstractFileClassLoader(storageForCompiledClasses, this.getClass.getClassLoader)

  /** Compiles the code as a class into the class loader of this compiler.
    *
    * @param code
    *   the class to be compile
    */
  ...
  /** Compiles the code-snippet as a class into the class loader of this compiler.
    *
    * @param code
    *   the snipped to be compile
    * @return
    *   the class resulting from the compilation
    */
  ...
  /** Checks if the given classname is available in the current classloader.
    ...
    * @return
    *   the class looked for if it exists
    */
  ...
  /** Generate a classname based on the hashcode of the code.
    *
    * @param code
    *   the code be used for generating the hashcode
    * @return
  ...
  /** Wrap source code in a new class with an apply method.
    ...
    * @param code
    *   the code snipped that should be wrapped into a class
    * @return
    *   the resulting, wrapped code
    */
